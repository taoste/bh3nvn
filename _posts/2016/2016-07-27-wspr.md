...
加入WSPR网络，可以让你通过搜索http://wsprnet.org/的数据库了解学习不同季节，不同时间，不同波段的传播规律。    
...
通过http://wsprnet.org/了解到，QRP Labs出品的Ultimate3/3S QRSS/WSPR信标套件比较流行，低廉的成本，合适的功率。另外，Raspberry Pi加上个LPF也是不错的选择，但是GPIO的RF功率只有差不多10mW，对于天线不给力的朋友，挑战性较大。
